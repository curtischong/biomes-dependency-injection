...
export function useEffectAsync(func: () => Promise<void>, deps?: DependencyList) {
    return useEffect(() => {
        void func();
    }, deps);
}
...
export function useEffectAsyncFetcher<T>(
    ...
    return useEffect(() => {
        ...
        fetcher(controller.signal)
            ...
            .catch((error) =>
                console.error("Error in async data fetcher", {
                    error,
                })
            );
...
export function useStateWithTimeRevert<T>(timeout: number, value?: T, onRevert?: () => any) {
...
export function useShowingTemporaryURL(url: string | undefined, deps: DependencyList = []) {
    useEffect(() => {
        if (url === undefined) {
            return;
        }
...
export function useTimeout(callback: (args: void) => void, ms: number, deps: DependencyList) {
