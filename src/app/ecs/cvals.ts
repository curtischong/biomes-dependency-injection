import { Averager } from "./counters";
import type { JSONable, RecursiveJSONable } from "./type_helpers";
...
export function remove(cvalDatabase: CvalDatabase, path: CvalPath) {
  ...
  if (existingItem.kind !== "CvalDatabase") {
    throw new Error("Expected an internal node but found an existing leaf node.");
  }
...
export function get(cvalDatabase: CvalDatabase, path: CvalPath): CvalNode | undefined {
  if (path.length == 0) {
    throw new Error("path cannot be empty!");
  }
...
export function getCvalHook(cvalDatabase: CvalDatabase, path: CvalPath): CvalHook<JSONable> | undefined {
...
export class AccumulatorContext {
  private accumulators: Map<CvalHook<JSONable>, Accumulator<JSONable, JSONable>> = new Map();
...
export function removeCvalPath({ path, cvalDatabase = undefined }: { path: CvalPath; cvalDatabase?: CvalDatabase }) {
...
type WidenedPrimitive<T> = T extends string ? string : T extends number ? number : T extends boolean ? boolean : T;
