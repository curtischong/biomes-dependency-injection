...
export function takeIf<T>(input: Iterable<T>, size: number, predicate: (value: T) => boolean) {
  const ret = [];
  if (size > 0) {
    for (const item of input) {
      if (predicate(item)) {
...
export function withSwappedLocations<T>(arr: Array<T>, index1: number, index2: number) {
  const ret = [...arr];
  [ret[index1], ret[index2]] = [ret[index2], ret[index1]];
  return ret;
}

export function combineCountMap<K>(base: Map<K, number>, newOne: Map<K, number>) {
...
export function getOrCreate<Key, Value>(map: Map<Key, Value>, key: Key, factory: () => Value) {
  if (!map.has(key)) {
    map.set(key, factory());
  }
  return map.get(key)!;
...
export function setMapFrom<Key, Value>(target: Map<Key, Value>, source: ReadonlyMap<Key, Value>) {
  target.clear();
...
export function filterMap<Key, Value>(input: ReadonlyMap<Key, Value>, predicate: (value: Value, key: Key) => boolean) {
...
export function filterMapInPlace<Key, Value>(map: Map<Key, Value>, predicate: (value: Value, key: Key) => boolean) {
...
export function onlyMapValue<Key, Value>(input: ReadonlyMap<Key, Value>): Value {
  ok(input.size === 1);
  return input.values().next().value;
}

export function anyMapValue<Key, Value>(input: ReadonlyMap<Key, Value>): Value | undefined {
...
export function mutUpdateMap<Key, Value>(input: Map<Key, Value>, fn: (value: Value, key: Key) => Value) {
...
export function filterSet<KT>(input: ReadonlySet<KT>, predicate: (val: KT) => boolean): KT[] {
  const result: KT[] = [];
  for (const item of input) {
    if (predicate(item)) {
      result.push(item);
...
export function mapSet<KT, RT>(input: ReadonlySet<KT>, fn: (value: KT, idx: number) => RT): RT[] {
  return Array.from(input, fn);
}

export function maybeFirstSetValue<Key>(input?: ReadonlySet<Key>): Key | undefined {
...
export async function tryAny<T>(...promises: Array<Promise<T>>): Promise<T | undefined> {
  const promiseMap = new Map(
    promises.map((promise, index) => [
      index,
      (async (promise, index) => [index, await promise] as [number, T])(promise, index),
    ])
  );
...
export class UniqueMultiMap<Key, Value> {
  ...
  [Symbol.iterator]() {
    return (this.underlyingMap as Map<Key, ReadonlySet<Value>>)[Symbol.iterator]();
  }
}
...
export function* mapIterable<T, R>(input: Iterable<T>, fn: (value: T, idx: number) => R) {
  let idx = 0;
...
export function* filterIterable<T>(input: Iterable<T>, predicate: (value: T, idx: number) => boolean) {
  let idx = 0;
  for (const value of input) {
...
export function* chooseAllPossibilities<T>(...possibilities: T[][]): Generator<T[]> {
...
export function compactMap<T, R>(it: Iterable<T> | undefined, fn: (t: T, index: number) => R | Falsey): R[] {
  const results: R[] = [];
  if (it) {
    let index = 0;
