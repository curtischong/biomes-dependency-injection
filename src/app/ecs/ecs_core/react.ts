...
export class ReactResources<P extends PathMap<P>> {
    ...
    useAll<K1 extends Key<P>, K2 extends Key<P>, K3 extends Key<P>, K4 extends Key<P>>(
        path1: [K1, ...Args<P, K1>],
        path2: [K2, ...Args<P, K2>],
        path3: [K3, ...Args<P, K3>],
        path4: [K4, ...Args<P, K4>]
    ...
    >(
        ...
        path6: [K6, ...Args<P, K6>],
        path7: [K7, ...Args<P, K7>]
    ): [Ret<P, K1>, Ret<P, K2>, Ret<P, K3>, Ret<P, K4>, Ret<P, K5>, Ret<P, K6>, Ret<P, K7>];
    useAll<
        K1 extends Key<P>,
        K2 extends Key<P>,
        K3 extends Key<P>,
    ...
    useAll<A extends AllKey<P>>(...paths: any[]) {
        ...
        useEffect(() => {
            ...
            return () => {
                bundles.forEach((args, i) => this.emitter.removeListener(this.key(args), cbs[i]));
                reactEmitterListeners.value -= bundles.length;
            };
        });
    ...
    maybeUse<K extends Key<P>>(cond: boolean, path: K, ...args: [...Args<P, K>]) {
        // Add path to list of listened to paths.
        const [version, setVersion] = useState(this.resources.version(path, ...args));
        useEffect(() => {
            if (!cond) {
                return;
            }
    ...
    useResolved<K extends Key<P>>(
        ...
        const [val, setVal] = useState<Awaited<typeof promise> | undefined>(undefined);
